...
namespace RE
{
	...
	public:
		...
		// add
		virtual void SetFollowNode(const RE::NiPointer<NiAVObject>& node) = 0;  // 01
	};
	...
	public:
		...
		void SetFollowNode(const RE::NiPointer<NiAVObject>& node) override;  // 01

		// members
		IMovementControllerRegisterInterface* register_interface{ nullptr };  // 28
		FormID                                actorID{ 0 };                   // 30
		ActorHandle                           actorHandle;                    // 34
		RE::NiPointer<NiAVObject>             node;                           // 38
